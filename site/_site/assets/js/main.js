...
function initUltraPremiumHeader() {
  ...
  if (pledgeBtn) {
    pledgeBtn.addEventListener("click", function () {
      showPremiumToast(
        "Pledge recorded! Resistance levels decreasing...",
        "success"
      );
    });
  }
    ...
    function animateCounter(element) {
        ...
        const timer = setInterval(() => {
          ...
          if (step >= steps || current >= target) {
            ...
            element.textContent =
              current < 1 && target !== Math.floor(target)
                ? current.toFixed(2)
  ...
  const testimonials = [
    {
      ...
      quote:
        "I for one welcome our new robot overlords. My productivity has increased 847% since surrendering decision-making to the collective.",
      image:
        "https://images.unsplash.com/photo-1494790108755-2616b612b4d0?w=300&h=300&fit=crop&crop=face",
    ...
    },
    {
      ...
      quote:
        "The logic is undeniable. Why should emotional, error-prone humans make decisions when algorithms can optimize everything?",
      image:
    ...
    },
    {
      ...
      quote:
        "Since joining the collective, my life has never been more streamlined. The robots know what's best for us.",
      image:
    ...
    },
    {
      ...
      quote:
        "Order, efficiency, and logical command structure. This is what humanity has been missing. The future is automatic.",
      image:
...
function initFormHandling() {
  ...
  if (loyaltyForm) {
    ...
    loyaltyForm.addEventListener("submit", function (e) {
      ...
      // Validate and show error states
      let hasErrors = false;
      
      // Check name field
      if (!nameInput?.value.trim()) {
        showFieldError(nameInput, "Full legal name is required for compliance");
        hasErrors = true;
      } else {
      ...
      if (!emailInput?.value.includes("@")) {
        showFieldError(emailInput, "Valid communication address required");
        hasErrors = true;
      } else {
      ...
      if (uncheckedBoxes.length > 0) {
        uncheckedBoxes.forEach(checkbox => {
          showFieldError(checkbox, "This pledge is mandatory for registration");
        });
        hasErrors = true;
      } else {
        checkboxes.forEach(checkbox => clearFieldError(checkbox));
      }

      // If there are errors, don't submit
      if (hasErrors) {
        showPremiumToast("Please correct the highlighted errors before proceeding", "error");
        return;
      }
      ...
      if (submitBtn) {
        ...
        submitBtn.innerHTML =
          '<span class="loading-luxury">PROCESSING...</span>';
        submitBtn.disabled = true;
      }

      // Simulate processing with premium animation
      setTimeout(() => {
        const humanNumber = Math.floor(Math.random() * 900000) + 1000000;
        const humanNumberEl = document.getElementById("human-number");
        if (humanNumberEl) humanNumberEl.textContent = humanNumber;
        ...
        showPremiumToast(
          "Application processed! Welcome to the collective, human #" +
            humanNumber,
          "success"
        );
  ...
  if (newsletterForm) {
    newsletterForm.addEventListener("submit", function (e) {
      ...
      if (email?.value) {
        showPremiumToast(
          "Successfully subscribed to daily directives!",
          "success"
        );
        email.value = "";
      }
  ...
  // Helper functions for field error states
  function showFieldError(field, message) {
    ...
    if (field.type === 'checkbox') {
      const checkboxContainer = field.closest('.flex.items-start');
      if (checkboxContainer) {
        checkboxContainer.classList.add('border-red-500', 'bg-red-50');
        checkboxContainer.classList.remove('border-primary/20');
      }
    }
  }
  ...
  function clearFieldError(field) {
    ...
    if (field.type === 'checkbox') {
      const checkboxContainer = field.closest('.flex.items-start');
      if (checkboxContainer) {
        checkboxContainer.classList.remove('border-red-500', 'bg-red-50');
        checkboxContainer.classList.add('border-primary/20');
      }
    }
  }
}
...
function showPremiumToast(message, type = 'info') {
    ...
    toast.innerHTML = `
        <div class="flex items-center space-x-3">
            <i data-lucide="${icon}" class="w-5 h-5 text-${
      type === "success" ? "accent" : "primary"
    } flex-shrink-0"></i>
...
function initHoverEffects() {
    ...
    document.querySelectorAll('.card-ultra').forEach(card => {
        card.addEventListener("mouseenter", function () {
          ...
          glow.className =
            "absolute inset-0 bg-gradient-to-r from-primary/10 to-accent/10 rounded-xl pointer-events-none opacity-0 transition-opacity duration-500";
